...
impl TextRunSegment {
    ...
    fn layout_into_line_items(&self, text_run: &TextRun, ifc: &mut InlineFormattingContextState) {
        for (run_index, run) in self.runs.iter().enumerate() {
            ...
            if run_index != 0 || self.break_at_start {
...
impl TextRun {
    ...
    pub(super) fn layout_into_line_items(&self, ifc: &mut InlineFormattingContextState) {
        ...
        for segment in self.shaped_text.iter() {
            segment.layout_into_line_items(self, ifc);
        }
    }
}
